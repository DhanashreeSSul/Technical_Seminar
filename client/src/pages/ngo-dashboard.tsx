...
export default function NgoDashboard() {
  ...
  const handleSubmitEvent = (e: React.FormEvent) => {
    ...
    const data = {
      ...eventForm,
      skillsRequired: eventForm.skillsRequired ? eventForm.skillsRequired.split(",").map(s => s.trim()) : [],
  ...
  const EventFormContent = () => (
    <form onSubmit={handleSubmitEvent} className="space-y-4">
      ...
      <DialogFooter>
        <Button type="submit" disabled={!eventForm.title || createEventMutation.isPending || updateEventMutation.isPending} data-testid="button-save-event">
                    ...
                    {events.map(event => (
                      <div key={event.id} className="flex items-center justify-between p-4 rounded-lg border bg-card">
                        ...
                        <div className="flex items-center gap-2">
                          <Button variant="ghost" size="icon" onClick={() => handleEditEvent(event)} data-testid={`button-edit-event-${event.id}`}>
                            <Edit className="h-4 w-4" />
                          </Button>
                          <Button variant="ghost" size="icon" onClick={() => deleteEventMutation.mutate(event.id)} data-testid={`button-delete-event-${event.id}`}>
